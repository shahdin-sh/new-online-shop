    ...
    if request.method == 'POST':
        ...
        if comment_form.is_valid():
            ...
            if request.user.is_authenticated:
                ...
                if not parent_obj:
                    msg = _('Opinion added successfully {username}').format(username=request.user.username)
                else:
                    msg = _('Reply added successfully {username}').format(username=request.user.username)
                ...
                msg = _('Opinion added successfully {guest_name}').format(guest_name=guest_data['name'])
            ...
            messages.error(request, f'{comment_form.errors}')
    else:
...
def add_to_wishlist(request, product_slug):
    ...
    if not product_detail.user_wished_product.filter(id=request.user.id).exists():
        ...
        messages.success(request, _('{product_name} add to your wishlist successfuly.').format(product_name=product_detail.name))
...
def remove_from_wishlist(request, product_slug):
    ...
    if product_detail.user_wished_product.filter(id=request.user.id).exists():
        ...
        messages.success(request, _('{product_name} remove from your wishlist successfuly.').format(product_name=product_detail.name))
